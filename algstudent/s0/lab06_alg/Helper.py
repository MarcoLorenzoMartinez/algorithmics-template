...
def fullMatrixFromFile(file):
    matrix = triangularMatrixFromFile(file)
    n = len(matrix)
    
    return fullMatrix(n, matrix)

def fullMatrix(n, matrix):
    # Iterate over the rows of the upper triangle
    for i in range(n):
        # Iterate over the columns of the upper triangle
        for j in range(i+1, n):
            # Mirror value across the diagonal
